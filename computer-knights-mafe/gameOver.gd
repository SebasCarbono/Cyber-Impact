...
func _intro_text_final():
	_append_line("\n[b]RECORRIDO ERRÓNEO, NO PUDISTE ENTRAR AL SISTEMA[/b]")
	_append_line("No recorriste de forma correcta los nodos para poder entrar.")
	_append_line("Elige una ruta:")
	_append_line("  • [color=yellow]Reiniciar[/color] - restart")
	_append_line("  • [color=yellow]Salir[/color] - exit\n")
...
func _handle_choice(cmd: String):
	match cmd.to_lower():
		"restart":
			_append_line("[color=green]Reiniciando...[/color]")
			await get_tree().create_timer(0.5).timeout
			get_tree().change_scene_to_file("res://escenas/Escena_principal.tscn")


		"exit":
			_append_line("[color=green]Saliendo...[/color]")
			await get_tree().create_timer(0.5).timeout
			get_tree().change_scene_to_file("res://Scenes/MENU.tscn")

		_:
			_append_line("[color=red]Entrada inválida.[/color] Escribe Reiniciar o Salir.")
